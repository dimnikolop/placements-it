...
@section('title', 'Νέος Ασκούμενος')

@section('content')
<div class="container">
    <div class="page-header">
        <div class="page-header-title">
            <h5>Create Trainee</h5>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb p-0">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="fas fa-home fa-fw"></i></a></li>
                <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('trainee.create') }}">New Trainee</a></li>
            </ol>
        </nav>
    </div>
    <div class="card custom-admin-card">
        <div class="card-header">
            <h5>Εισαγωγή νέου ασκούμενου φοιτητή</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('trainee.store') }}" method="post">
                ...
                <div class="form-row mb-4">
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control @error('phone') is-invalid @enderror" type="text" id="inputPhone" name="phone" value="{{ old('phone') }}" placeholder="(προαιρετικό)"/>
                        @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-row mb-4">
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control @error('supervisor') is-invalid @enderror" type="text" id="inputSupervisor" name="supervisor" value="{{ old('supervisor') }}"/>
                        @error('supervisor')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control @error('job') is-invalid @enderror" type="text" id="inputJob" name="job" value="{{ old('job') }}"/>
                        @error('job')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
